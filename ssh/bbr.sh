...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/clock69/scvps/main > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/clock69/scvps/main | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/clock69/scvps/main | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
